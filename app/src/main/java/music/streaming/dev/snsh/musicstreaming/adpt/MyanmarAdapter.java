...
public class MyanmarAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    public final int TYPE_DATA = 0;
    public final int TYPE_LOAD = 1;

    static Context context;
    List<DiscoverMyanmar.Data> myanmarDataList;
    public OnLoadMoreListener loadMoreListener;
    boolean isLoading = false, isMoreDataAvailable = true;

    public MyanmarAdapter(Context context, List<DiscoverMyanmar.Data> myanmarDataList) {
        this.context = context;
        this.myanmarDataList = myanmarDataList;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (getItemViewType(position) == TYPE_DATA) {
            ((MovieHolder) holder).bindData(myanmarDataList.get(position));
        }
    ...
    public int getItemViewType(int position) {
        if (myanmarDataList.get(position).type == null) {
            return TYPE_DATA;
        } else {
            if (myanmarDataList.get(position).type.equals("load")) {
                return TYPE_LOAD;
            } else {
                return TYPE_DATA;
            }
    ...
    static class MovieHolder extends RecyclerView.ViewHolder {
        ...
        void bindData(DiscoverMyanmar.Data data) {
