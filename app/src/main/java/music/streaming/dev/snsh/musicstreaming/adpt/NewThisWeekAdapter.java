...
public class NewThisWeekAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    public final int TYPE_DATA = 0;
    public final int TYPE_LOAD = 1;

    static Context context;
    List<DiscoverNewThisWeek.Data> weekDataList;
    public OnLoadMoreListener loadMoreListener;
    boolean isLoading = false, isMoreDataAvailable = true;

    public NewThisWeekAdapter(Context context, List<DiscoverNewThisWeek.Data> weekDataList) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (getItemViewType(position) == TYPE_DATA) {
            ((MovieHolder) holder).bindData(weekDataList.get(position));
        }
    ...
    public int getItemViewType(int position) {
        if (weekDataList.get(position).type == null) {
            return TYPE_DATA;
        } else {
            if (weekDataList.get(position).type.equals("load")) {
                return TYPE_LOAD;
            } else {
                return TYPE_DATA;
            }
    ...
    static class MovieHolder extends RecyclerView.ViewHolder {
        ...
        void bindData(DiscoverNewThisWeek.Data data) {
