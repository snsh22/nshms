...
public class KoreaAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    public final int TYPE_DATA = 0;
    public final int TYPE_LOAD = 1;

    static Context context;
    List<DiscoverKorea.Data> koreaDataList;
    public OnLoadMoreListener loadMoreListener;
    boolean isLoading = false, isMoreDataAvailable = true;

    public KoreaAdapter(Context context, List<DiscoverKorea.Data> koreaDataList) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (getItemViewType(position) == TYPE_DATA) {
            ((MovieHolder) holder).bindData(koreaDataList.get(position));
        }
    ...
    public int getItemViewType(int position) {
        if (koreaDataList.get(position).type == null) {
            return TYPE_DATA;
        } else {
            if (koreaDataList.get(position).type.equals("load")) {
                return TYPE_LOAD;
            } else {
                return TYPE_DATA;
            }
    ...
    static class MovieHolder extends RecyclerView.ViewHolder {
        ...
        void bindData(DiscoverKorea.Data data) {
