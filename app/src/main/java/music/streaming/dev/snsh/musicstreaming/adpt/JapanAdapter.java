...
public class JapanAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    public final int TYPE_DATA = 0;
    public final int TYPE_LOAD = 1;

    static Context context;
    List<DiscoverJapan.Data> japanDataList;
    public OnLoadMoreListener loadMoreListener;
    boolean isLoading = false, isMoreDataAvailable = true;

    public JapanAdapter(Context context, List<DiscoverJapan.Data> japanDataList) {
        this.context = context;
        this.japanDataList = japanDataList;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (getItemViewType(position) == TYPE_DATA) {
            ((MovieHolder) holder).bindData(japanDataList.get(position));
        }
    ...
    public int getItemViewType(int position) {
        if (japanDataList.get(position).type == null) {
            return TYPE_DATA;
        } else {
            if (japanDataList.get(position).type.equals("load")) {
                return TYPE_LOAD;
            } else {
                return TYPE_DATA;
            }
    ...
    static class MovieHolder extends RecyclerView.ViewHolder {
        ...
        void bindData(DiscoverJapan.Data data) {
